...
from patterpunk.config import DEFAULT_TEMPERATURE, OPENAI_MAX_RETRIES
...
class OpenAiModel(Model):
    ...
    def generate_assistant_message(
        ...
        while not done and retry_count < OPENAI_MAX_RETRIES:
            try:
                response = openai.ChatCompletion.create(**openai_parameters)
        ...
        if response_message['finish_reason'] == 'function_call':
            return FunctionCallMessage(response_message['message']['content'], response_message['message']['function_call'])
        else:
