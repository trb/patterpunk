...
def extract_json(json_string: str) -> List[str]:
    ...
    for i, char in enumerate(json_string):
        if char in ['{', '['] and not inside_string and (not bracket_type or bracket_type == char):
            if not stack:
                start_index = i
                bracket_type = char
            stack.append(char)
        elif char in ['}', ']'] and not inside_string and stack and bracket_type == '{' if char == '}' else bracket_type == '[':
            stack.pop()
            if not stack:
...
class Chat:
    ...
    def extract_json(self):
        ...
        if re.search(r'\{.*}', chat_text, re.IGNORECASE | re.DOTALL) or re.search(r'\[.*]', chat_text, re.IGNORECASE | re.DOTALL):
