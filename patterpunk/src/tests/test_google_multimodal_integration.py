...
def test_google_multimodal_text_and_image_generation():
    ...
    response = (
        chat.add_message(
            UserMessage([
                TextChunk("Look at this image and generate a detailed description, then create an image of a red apple. Provide both text description and generated image."),
    ...
    if isinstance(response.content, str):
        ...
        text_chunks = [chunk for chunk in response.content if isinstance(chunk, (str, TextChunk))]
        image_chunks = [chunk for chunk in response.content if isinstance(chunk, MultimodalChunk)]
