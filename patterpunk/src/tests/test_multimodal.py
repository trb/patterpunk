...
def test_bytes():
    ...
    with open(get_resource("research.pdf"), "rb") as f:
        assert (
            multimodal_chunk.to_bytes() == f.read()
        ), "Bytes from MultimodalChunk does not match the original file."
...
def test_download_invalid_url():
    chunk = MultimodalChunk.from_url(
        "http://invalid-host-that-does-not-exist.com/file.txt"
    )
