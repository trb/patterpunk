...
from patterpunk.llm.messages import Message, SystemMessage, UserMessage, AssistantMessage


class TestCacheChunk:
    def test_cache_chunk_creation(self):
        chunk = CacheChunk(content="test content", cacheable=True, ttl=timedelta(hours=1))
...
class TestBackwardCompatibility:
    ...
    def test_message_repr_with_chunks(self):
        chunks = [CacheChunk(content="long content that should be truncated", cacheable=True)]
        message = UserMessage(content=chunks)
...
class TestCacheChunkUsagePatterns:
    def test_mixed_cacheable_chunks(self):
        chunks = [
            ...
            CacheChunk(content="Large document content...", cacheable=True, ttl=timedelta(hours=1)),
            CacheChunk(content=f"Query: What is the main theme?", cacheable=False)
        ]
        message = UserMessage(content=chunks)
        
        assert message.has_cacheable_content()
        assert message.get_content_as_string() == "Static instructions: Large document content...Query: What is the main theme?"
