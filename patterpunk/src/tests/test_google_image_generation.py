...
def verify_image_content(image_chunk: MultimodalChunk, expected_features: List[str], should_not_contain: List[str] = None) -> tuple[bool, str]:
    if not OPENAI_API_KEY:
        pytest.skip("OpenAI credentials not available for image verification")
    
    verifier_model = OpenAiModel(
        model="gpt-4o",
        temperature=0.1
    )
...
def test_google_generate_new_image():
    ...
    response = (
        chat.add_message(
            SystemMessage("You are an expert digital artist who creates vivid, detailed images based on descriptions.")
        )
        .add_message(
            UserMessage(
                content=[
                    TextChunk("""Create a detailed, photorealistic image of a futuristic floating city at sunset. 
    ...
    assert hasattr(message, '_raw_content'), "AssistantMessage should have _raw_content attribute"
    assert isinstance(message._raw_content, list), "Raw content should be a list for multimodal response"
    
    text_chunks = message.texts
    image_chunks = message.images
    
    assert len(text_chunks) > 0, "Response should contain descriptive text about the generated image"
    ...
    for image_chunk in image_chunks:
        assert isinstance(image_chunk, MultimodalChunk), f"Image chunk should be MultimodalChunk, got {type(image_chunk)}"
        assert image_chunk.media_type is not None, "Image should have a media type"
        assert image_chunk.media_type.startswith("image/"), f"Media type should be image/*, got {image_chunk.media_type}"
        
        image_bytes = image_chunk.to_bytes()
        assert len(image_bytes) > 1000, "Image should have substantial data (at least 1KB)"
        ...
        is_valid_image = any(image_header.startswith(header) for header in valid_headers)
        assert is_valid_image, f"Image should have valid image file header, got {image_header[:4]}"
        
        print(f"Generated futuristic city image: {len(image_bytes)} bytes, header={image_header[:4]}")
        ...
        assert verification_success, f"Generated image does not match expected futuristic floating city. Analysis: {verification_analysis}"
    
    combined_text = message.content
    assert "city" in combined_text.lower() or "futuristic" in combined_text.lower() or "floating" in combined_text.lower(), \
...
def test_google_edit_existing_image_ducks_to_pelicans():
    ...
    response = (
        chat.add_message(
            SystemMessage("""You are an expert photo editor who can seamlessly modify images while 
    ...
    assert len(text_chunks) > 0, "Response should contain text describing the edits made"
    assert len(image_chunks) > 0, "Response should contain the edited image with pelicans"
    ...
    for text_chunk in text_chunks:
        if isinstance(text_chunk, TextChunk):
            ...
            text_content = text_chunk
        assert len(text_content) > 10, "Text description should describe the editing process"
    
    combined_text = message.content.lower()
    assert "pelican" in combined_text or "replaced" in combined_text or "edit" in combined_text or "change" in combined_text, \
        "Response text should mention pelicans or the editing/replacement process"
    
    for idx, image_chunk in enumerate(image_chunks):
        assert isinstance(image_chunk, MultimodalChunk), f"Image chunk should be MultimodalChunk, got {type(image_chunk)}"
        assert image_chunk.media_type is not None, "Edited image should have a media type"
        assert image_chunk.media_type.startswith("image/"), f"Media type should be image/*, got {image_chunk.media_type}"
        
        image_bytes = image_chunk.to_bytes()
        assert len(image_bytes) > 1000, "Edited image should have substantial data (at least 1KB)"
        ...
        is_valid_image = any(image_header.startswith(header) for header in valid_headers)
        assert is_valid_image, f"Edited image should have valid image file header, got {image_header[:4]}"
        
        print(f"Edited pelican image {idx}: {len(image_bytes)} bytes, header={image_header[:4]}")
        ...
        assert verification_success, f"Edited image does not show pelicans instead of ducks. Analysis: {verification_analysis}"
...
def test_google_multimodal_generation_with_multiple_images():
    ...
    response = (
        chat.add_message(
            SystemMessage("You are a creative visual storyteller who creates illustrated stories with multiple images.")
        )
        .add_message(
            UserMessage(
                content=[
                    TextChunk("""Create a short illustrated story about a day in the life of a robot chef. 
                    Include:
    ...
    assert len(image_chunks) >= 2, "Response should contain multiple images for the story"
    
    print(f"Generated story with {len(text_chunks)} text segments and {len(image_chunks)} images")
    
    for idx, image_chunk in enumerate(image_chunks):
        assert isinstance(image_chunk, MultimodalChunk)
        image_bytes = image_chunk.to_bytes()
        assert len(image_bytes) > 1000, f"Story image {idx} should have substantial data"
        ...
        is_valid_image = any(image_header.startswith(header) for header in valid_headers)
        assert is_valid_image, f"Story image {idx} should have valid image file header, got {image_header[:4]}"
        ...
        verification_success, verification_analysis = verify_image_content(
            ...
            should_not_contain=[
                ...
                "Dark or horror theme"
            ]
        )
        
        print(f"\nStory image {idx} verification analysis:\n{verification_analysis}")
        assert verification_success, f"Story image {idx} does not match robot chef theme. Analysis: {verification_analysis}"
    
    combined_text = message.content.lower()
    assert "robot" in combined_text or "chef" in combined_text or "cooking" in combined_text, \
