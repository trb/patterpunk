"""
Tests for async streaming support with OpenAI.

These tests validate the complete_stream() interface and streaming behavior
with the OpenAI provider using the Responses API.
"""
...
# =============================================================================
# These tests require a reasoning model (o3-mini, o1, etc.)
...
async def test_stream_thinking_and_content():
    ...
    NOTE: OpenAI reasoning models may not always produce thinking summaries,
    ...
    async with chat.complete_stream() as stream:
        # First iterate through thinking (may be empty for OpenAI)
    ...
    # Note: OpenAI reasoning models may or may not produce thinking summaries
...
async def test_stream_with_reasoning_and_tool_call():
    ...
    NOTE: Unlike Anthropic's interleaved thinking, OpenAI reasoning models
    ...
    This test verifies:
    1. Reasoning models can use tools during streaming
    ...
    async with chat.complete_stream() as stream:
        # Note: OpenAI reasoning summaries come at the end, not interleaved
    ...
    chat = Chat(
        model=OpenAiModel(
            ...
            thinking_config=ThinkingConfig(
                effort="medium"
            ),  # Use medium to encourage reasoning
        )
    )
