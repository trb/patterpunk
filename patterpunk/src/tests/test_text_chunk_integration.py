...
def test_text_chunk_templating_with_mixed_types():
    ...
    prepared = message.prepare({"name": "Alice", "age": 30, "city": "New York"})
...
def test_text_chunk_provider_conversion(model_class):
    if model_class == AnthropicModel:
        ...
        openai_content = model._convert_message_content_for_openai_responses(
            content, role="user"
        )
...
def test_backward_compatibility():
    old_message = UserMessage(
        [CacheChunk("Hello", cacheable=False), CacheChunk("world", cacheable=True)]
    )

    new_message = UserMessage([TextChunk("Hello"), CacheChunk("world", cacheable=True)])
