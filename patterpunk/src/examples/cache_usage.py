...
def basic_caching_example():
    ...
    system_msg_new = SystemMessage(
        [
            CacheChunk(
                "You are a helpful assistant with expertise in:", cacheable=False
            ),
            CacheChunk(
                "Python programming, data science, web development...", cacheable=True
            ),
        ]
    )
    ...
    user_msg_new = UserMessage(
        [
            CacheChunk(
                "Large document context goes here...",
                cacheable=True,
